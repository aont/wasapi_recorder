...
HRESULT LoopbackCapture(
    ...
    for (int i=0; ; i++){
        ...
        MMIOINFO mi = {0};
        //int i = 0;
        wchar_t outputFileName[128];
        wsprintf(outputFileName, fileNameTemplate.c_str(), i);
        
        wchar_t dbgOutputFileName[128];
        wsprintf(dbgOutputFileName, L"debug%05d.wav", i);
        FILE* const fp = _wfopen(dbgOutputFileName, L"wb");
        if (fp==NULL) {
            ERR(L"fopen failed", "");
        }
        // FcloseOnExit fcloseOnExit(fp);
        HMMIO hFile = mmioOpen(
            // some flags cause mmioOpen write to this buffer
            // but not any that we're using
            const_cast<LPWSTR>(outputFileName),
            &mi,
            MMIO_WRITE | MMIO_CREATE
        );
        if (NULL == hFile) {
            ERR(L"mmioOpen(\"%ls\", ...) failed. wErrorRet == %u", outputFileName, mi.wErrorRet);
            return E_FAIL;
        }
        // FcloseOnExit fcloseOnExit(hFile);
        LOG(L"file=\"%ls\"", outputFileName);
        ...
        *pnFrames = 0;
        ...
        for (UINT32 nPasses = 0; !bDone; nPasses++) {
            ...
            for (
                ...
                fwrite((char*)pData, lBytesToWrite, 1, fp);
        ...
        {
            MMRESULT hr = FinishWaveFile(hFile, &ckData, &ckRIFF);
            if (FAILED(hr)) {
        ...
        if(false){
            MMIOINFO mi = {0};
            hFile = mmioOpen(const_cast<LPWSTR>(outputFileName), &mi, MMIO_READWRITE);
            if (NULL == hFile) {
                ERR(L"mmioOpen(\"%ls\", ...) failed. wErrorRet == %u", outputFileName, mi.wErrorRet);
                return -__LINE__;
            }
        ...
        // descend into the RIFF/WAVE chunk
            ...
            LONG lBytesWritten = mmioWrite(
                hFile,
                reinterpret_cast<PCHAR>(*pnFrames),
                sizeof(*pnFrames)
            );
            if (lBytesWritten != sizeof(*pnFrames)) {
                ERR(L"Updating the fact chunk wrote %u bytes; expected %u", lBytesWritten, (UINT32)sizeof(*pnFrames));
                return -__LINE__;
            }
