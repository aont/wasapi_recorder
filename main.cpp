...
int main() {
    ...
    int argc_w;
    LPCWSTR* argv_w = (LPCWSTR*)CommandLineToArgvW(GetCommandLineW(), &argc_w);
    return do_everything(argc_w, argv_w);
}
...
int do_everything(int argc, LPCWSTR argv[]) {
    ...
    prefs.fileNameTemplate = L"output%05d.wav";
    ...
    threadArgs.fileNameTemplate = prefs.fileNameTemplate;
            ...
            case WAIT_OBJECT_0 + 1: // hStdIn
                ...
                if (!ReadConsoleInput(hStdIn, rInput, ARRAYSIZE(rInput), &nEvents)) {
                    ...
                    for (DWORD i = 0; i < nEvents; i++) {
                        // fprintf(stderr, "EventType:%d VirtualKeyCode:%d\n", rInput[i].EventType, rInput[i].Event.KeyEvent.wVirtualKeyCode);
                        if (
                            KEY_EVENT == rInput[i].EventType &&
                            /*VK_RETURN*/
                            81 == rInput[i].Event.KeyEvent.wVirtualKeyCode &&
